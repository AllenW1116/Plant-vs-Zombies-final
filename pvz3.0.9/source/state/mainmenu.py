...
class Menu(tool.State):
    ...
    def setupExit(self):
        frame = tool.GFX[c.MENU_EXIT]
        rect = frame.get_rect()
        frame_rect = (rect.x, rect.y, rect.w, rect.h)

        self.exit_image = tool.get_image(tool.GFX[c.MENU_EXIT], *frame_rect, c.BLACK, 1)
        self.exit_rect = self.exit_image.get_rect()
        self.exit_rect.x = 735
        self.exit_rect.y = 515
    ...
    def update(self, surface, current_time, mouse_pos, mouse_click):
        ...
        if mouse_pos and self.checkExitClick(mouse_pos):
            self.next = c.EXIT
            self.done = True
        elif not self.option_clicked:
            if mouse_pos:
